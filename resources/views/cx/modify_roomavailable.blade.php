  ...
  <body style="padding-top: 0px!important;">
   ...
   <div>
    ...
    <script type="text/javascript">
      ...
      {
         if(parseInt({{$newGuest}}) > parseInt(cp) && parseInt({{$newGuest}}) <= parseInt(tcp) && b == a )
          {
            $('#btn-submit').attr('disabled','true');
            swal({
              title: "Are you sure?",
              text: "You will have an extra charge of Php" + (({{$newGuest}} - cp ) * 450) +"\n"+
              "("+({{$newGuest}} - cp)+" extra person x 450)",
              icon: "warning",
              buttons: true,
              dangerMode: true,
            })
          ...
          }else if(parseInt({{$newGuest}}) > parseInt(tcp))
          {
            swal('Room Capacity Validation',"We can't accommodate the given number of guests even with the inclusion of "+(tcp - cp)+" mattresses to all room selected. "+({{$newGuest}} - tcp)+" excess guests.",'info')
            .then(willBack => {
              if(willBack)
              {
